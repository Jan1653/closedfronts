...
export class StructurePass {
  ...
  updateStructures(units: Map<number, UnitState>): void {
    ...
    for (const unit of units.values()) {
      ...
      // 0 = normal, 1 = under construction (light grey), 2 = deactivated by an
      // electric bomb or a disaster (darker, unmistakably dead grey). Both use
      // the grey branch in the shader; BarPass only draws a build bar for real
      // underConstruction, so a merely-disabled unit shows no progress bar.
      this.instanceBuf.float32[off + 3] = unit.disabled
        ? 2
